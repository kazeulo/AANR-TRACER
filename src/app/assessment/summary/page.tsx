...
function AnswerRow({ q, answer }: { q: Question; answer: unknown }) {
  ...
  if (qType === "dropdown") {
    ...
    return (
      <li className="flex items-start gap-3 py-2.5">
        <div className={`flex-shrink-0 mt-0.5 w-5 h-5 rounded-[5px] border-2 flex items-center justify-center ${
          selected ? "bg-[#4aa35a] border-[#4aa35a]" : "bg-white border-[#e0dbd3]"
        }`}>
          {selected ? (
            <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
              <path d="M1 4l3 3 5-6" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-[13px] leading-relaxed ${selected ? "text-[#1a1a1a]" : "text-[#b0a99e]"} font-light`}>
            {q.questionText}
          </p>
          {selected ? (
            <span className="inline-flex items-center gap-1.5 mt-1.5 text-[11px] font-medium text-[#4aa35a] bg-[#4aa35a]/[0.08] border border-[#4aa35a]/20 px-2.5 py-0.5 rounded-full">
              <svg width="8" height="8" viewBox="0 0 8 8" fill="none">
  ...
  if (qType === "multi-conditional") {
    ...
    if (!mc?.selection) {
      return (
        <li className="flex items-start gap-3 py-2.5">
          <div className="flex-shrink-0 mt-0.5 w-5 h-5 rounded-[5px] border-2 border-[#e0dbd3] bg-white flex items-center justify-center">
            <svg width="8" height="2" viewBox="0 0 8 2" fill="none">
              <path d="M1 1h6" stroke="#c8c3b8" strokeWidth="1.5" strokeLinecap="round" />
            </svg>
          </div>
    ...
    return (
      <li className="flex items-start gap-3 py-2.5">
        <div className={`flex-shrink-0 mt-0.5 w-5 h-5 rounded-[5px] border-2 flex items-center justify-center ${
          isPositive ? "bg-[#4aa35a] border-[#4aa35a]" : "bg-amber-100 border-amber-300"
        }`}>
          {isPositive ? (
            <svg width="10" height="8" viewBox="0 0 10 8" fill="none">
              <path d="M1 4l3 3 5-6" stroke="white" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round" />
        ...
        <div className="flex-1 min-w-0">
          <p className="text-[13px] text-[#1a1a1a] font-light leading-relaxed">{q.questionText}</p>
          <span className={`inline-block mt-1.5 text-[11px] font-medium px-2.5 py-0.5 rounded-full border ${
            isPositive
              ? "text-[#4aa35a] bg-[#4aa35a]/[0.08] border-[#4aa35a]/20"
    ...
    <div className="bg-white border border-[#ede9e0] rounded-2xl overflow-hidden shadow-[0_2px_12px_rgba(15,46,26,0.04)]">
      ...
      <ul className="px-7 py-3 divide-y divide-[#f5f2ec]">
        ...
        <li className="flex items-start gap-3 py-2.5">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-[13px] font-light text-[#1a1a1a] leading-relaxed">
                  ...
                  return (
                    <li key={type} className="flex items-center gap-2.5">
                      <div className={`flex-shrink-0 w-4 h-4 rounded-[4px] border flex items-center justify-center ${
                        isDone ? "bg-[#4aa35a] border-[#4aa35a]" : "bg-white border-[#d0d7d4]"
                      }`}>
                        {isDone && (
                      ...
                      <span className="text-[12px] text-[#4a5568] font-light">{type}</span>
                      {status && (
                        <span className={`text-[10px] font-medium px-2 py-0.5 rounded-full ${
                          isDone
                            ? "bg-[#4aa35a]/10 text-[#4aa35a]"
                            : "bg-[#f5f2ec] text-[#94a3a0]"
                        }`}>
                          {status}
                        </span>
                      )}
      ...
      <div className="max-w-[860px] mx-auto">
        ...
        {/* <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-8">
          {[
            ...
            <div key={label} className={`bg-white border rounded-xl px-4 py-3 text-center shadow-[0_2px_8px_rgba(15,46,26,0.04)] ${
              highlight ? "border-amber-200" : "border-[#ede9e0]"
            }`}>
              <p className="text-[11px] font-bold tracking-[1.5px] uppercase text-[#94a3a0] mb-1">{label}</p>
              <p className={`text-[15px] font-semibold ${highlight ? "text-amber-600" : "text-[#0f2e1a]"}`}>{value}</p>
            </div>
          ))}
        </div> */}
