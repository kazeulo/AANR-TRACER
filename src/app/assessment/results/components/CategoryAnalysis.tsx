...
function buildInsight(scores: CategoryScore[], completedQuestions: QuestionItem[]): string {
  ...
  if (completedCategories.length === 0) {
    ...
    insight += strengthEntry.highestLevel > 0
      ? `, already addressing requirements aligned with TRACER Level ${strengthEntry.highestLevel}.`
      : `.`;
  ...
  return (
    <svg width={size} height={size} viewBox={`0 0 ${size} ${size}`}>
      {[0.25, 0.5, 0.75, 1].map(scale => (
        <polygon key={scale} points={polygonPoints(scale)} fill="none" stroke="#e8e4dc" />
      ))}
      {axes.map((ax, i) => (
        <line key={i} x1={cx} y1={cy} x2={ax.x} y2={ax.y} stroke="#e8e4dc" />
      ))}
      <polygon points={dataPoints} fill="rgba(74,163,90,0.15)" stroke="#4aa35a" strokeWidth="2" />
      {scores.map((s, i) => {
        const angle = (i * 2 * Math.PI) / n - Math.PI / 2;
        const r = radius * (s.percent / 100);
        return (
          <circle key={i}
            ...
            r={4} fill={getBarColor(s.percent)} stroke="white"
          />
        );
      })}
      {labels.map((l, i) => (
        <text key={i} x={l.x} y={l.y} textAnchor="middle" fontSize="9">{l.label}</text>
      ))}
    </svg>
  );
}
...
export default function CategoryAnalysis({
  ...
  const coreLevels       = categoryLevels.filter(s =>  CORE_CATEGORIES.has(s.category));
  const supportingLevels = categoryLevels.filter(s => !CORE_CATEGORIES.has(s.category));

  // Strongest: prefer core; fall back to supporting only if all core are at 0
  const coreWithProgress = coreLevels.filter(s => s.highestLevel > 0);
  const strongest = coreWithProgress.length > 0
    ? coreWithProgress.reduce((a, b) => b.highestLevel > a.highestLevel ? b : a)
    : supportingLevels.filter(s => s.highestLevel > 0).length > 0
      ? supportingLevels.filter(s => s.highestLevel > 0)
          .reduce((a, b) => b.highestLevel > a.highestLevel ? b : a)
      : coreLevels[0]; // absolute fallback — at least render something

  // Focus: prefer incomplete core (excluding strongest); fall back to supporting
  // only once all core categories are fully complete.
  const allCoreComplete    = coreLevels.every(s => s.isComplete);
  const incompleteCore     = coreLevels.filter(s => !s.isComplete && s.category !== strongest.category);
  const incompleteSupporting = supportingLevels.filter(s => !s.isComplete);

  const focusCandidates = !allCoreComplete ? incompleteCore : incompleteSupporting;
  const weakest = focusCandidates.length > 0
    ? focusCandidates.reduce((a, b) => b.highestLevel < a.highestLevel ? b : a)
    : null; // null = all areas addressed

  const insight = useMemo(
    () => buildInsight(scores, completedQuestions),
    [scores, completedQuestions]
  );
      ...
      <div className="px-7 py-6 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-3">
          ...
          {weakest ? (
            ...
            <div className="rounded-xl px-4 py-3 border border-[#a5f3fc] bg-[#ecfeff]">
              <p className="text-[10px] font-bold tracking-[1.5px] uppercase text-[#164e63] mb-1">
                ✅ All Areas Addressed
              </p>
              <p className="text-[13px] font-semibold text-[#164e63] leading-snug">
                No critical gaps identified
              </p>
              <p className="text-[11px] text-[#0891b2] mt-0.5">
                Keep building toward full commercialization
              </p>
            </div>
          )}
        </div>
              ...
              <div key={s.category}>
                <div className="flex items-center justify-between mb-1.5">
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                    <span className="text-[11px] text-[#94a3a0]">{s.answered}/{s.total}</span>
                    <span
                ...
                <div className="h-2 w-full bg-[#f0ece3] rounded-full overflow-hidden">
                  <div
                    ...
                    style={{ width: `${s.percent}%`, backgroundColor: getBarColor(s.percent) }}
                  />
                </div>
