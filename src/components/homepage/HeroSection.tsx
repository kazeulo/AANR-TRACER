...
export default function HeroSection(){
  ...
  return(
    <section className="relative min-h-screen flex items-center bg-[var(--color-primary)] overflow-hidden px-6 lg:px-[6vw] pt-[70px] pb-[120px]">
      ...
      <div className="relative z-10 max-w-[1200px] mx-auto w-full grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
        ...
        {/* Left */}
        <div>
          <h1 className=" text-[clamp(50px,4.2vw,55px)] leading-[1.1] text-white mb-5 tracking-tight">
          ...
          {/* Partner Logos */}
          <div className="my-7 bg-[var(--white-35)] border border-white rounded-xl px-4 py-2 w-full max-w-[480px]">
            ...
            <div className="flex flex-wrap items-center justify-center gap-3">
              {PARTNER_LOGOS.map(({ src, alt, w, h }) => (
                <Image key={src} src={src} alt={alt} width={w} height={h} className="h-[32px] sm:h-[36px] w-auto object-contain opacity-80 hover:opacity-100 transition-opacity" />
              ))}
            </div>
          </div>
